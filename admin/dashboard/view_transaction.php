...
<?php
	...
	$allowed_txn = array("admin_credit","superwallet");
	$type = "";
	// CHECK AVAILABILITY
		if (isset($_GET['type'])) {
			...
			if (!in_array($type, $allowed_txn)) {
				...
				echo "Redirecting...Please Wait";
				header("Refresh:0, url=view_transaction.php");
				exit;
			}

			if ($type == "admin_credit") {
				$type_caps = "Admin Investment Summary";
			}
			if ($type == "superwallet") {
				$type_caps = "Member Superwallet Investment";
			}

			$header = "$type_caps";
			$check_type = " `fund_transaction`.`transaction_type`='$type' ";
		} else {
			$header = "Investment Pending For Approval";
			$check_type = " `fund_transaction`.`to_user_id`='COMPANY' ";
		}
	// CHECK AVAILABILITY
		...
		if(isset($_POST['submit'])){
			if(!empty($_POST['approve_transaction'])) {
				...
				foreach($_POST['approve_transaction'] as $key => $value) {
					...
					$count_already_approved = mysqli_num_rows(mysqli_query($conn, "SELECT `id` FROM `fund_transaction` WHERE `id`='$txn_id' AND `status`=1"));
									...
									if (mysqli_query($conn, "$query_insert_wallet_transaction")) {
										// UPDATE USER FUNDS AFTER APPROVING THE TRANSACTION AND INSERTING INTO WALLET TRANSACTION TABLE
											...
											foreach ($receiver_wallet_update_array as $row) {
												...
												$query_update_users_fund_total .= 
													"
														WHEN '$user_id_receiver' THEN `fund_wallet`+`fund_commission`
													";
						...
						<div class="row">
							<div class="col-12">
								...
								<div class="card">
									...
									<div class="card-body mt-4" style="padding:0.5rem;">
												...
												<tbody id="tableBody">
													...
													foreach ($records as $row) {
														...
														switch ($transaction_type) {
															...
															case 'superwallet':
																$user_txn_for = "Superwallet";
																$class_txn = "warning";
																break;
															
															case 'renewal':
																$user_txn_for = "RENEWAL FEE";
																$class_txn = "danger";
																break;
															
															case 'upgrade':
																$user_txn_for = "UPGRADE FEE";
																$class_txn = "info";
																break;
														...
														switch ($status) {
															case 2:
																$transaction_status = '<span class="badge bg-warning" style="white-space: normal;">PENDING</span>';
																break;
															
															case 1:
																$transaction_status = '<span class="badge bg-success" style="white-space: normal;">APPROVED</span>';
																break;
															
															case 0:
																$transaction_status = '<span class="badge bg-danger" style="white-space: normal;">REJECTED</span>';
																break;
														...
														?>
															...
															<tr>
																...
																<td class="text-center no_print">
																	<?php
																		if ($transaction_type == "fresh") {
																			...
																			?>
																				<span class="badge bg-danger">DIRECT INVEST</span>
																			<?php
																		}
																	?>
																</td>
																<td class="text-start"><?php echo $transaction_mode; ?></td>
																...
																<td class="has-html text-center"><?php echo $transaction_status; ?></td>
