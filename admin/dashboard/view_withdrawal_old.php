...
<?php
	...
	$header = "All Withdrawal Requests";
	...
	// GET TRANSACTION
		// FOR SELECT QUERIES
			$query_transaction = "SELECT `withdrawal`.`id`, `withdrawal`.`user_id`, `withdrawal`.`withdrawal_amount`, `withdrawal`.`status`, `withdrawal`.`comment`, `withdrawal`.`create_date`,`users`.`name`,`users`.`mobile`,`users`.`email`,`users`.`pan`,`users`.`bank_name`,`users`.`branch_name`,`users`.`account_no`,`users`.`ifs_code` FROM `withdrawal` LEFT JOIN `users` ON `users`.`user_id`=`withdrawal`.`user_id` WHERE 1 ORDER BY `withdrawal`.`create_date` DESC";
															...
															<tr>
																...
																<td class="no_print text-center">
																	<?php
                                                                        if ($status=='pending') {
                                                                            ?>
                                                                                <span class='badge bg-warning rounded-pill show-pointer' style="background: #E07C24 !important;" onclick="showNotif('Kindly Approve Txn From Pending Withdrawal List','default');">Pending</span>
																...
																<td class="has-html text-center"><span class="badge bg-info" style="white-space: normal;"><?php echo $withdrawal_amount; ?></td>
																...
																<td class="text-start"><?php echo $account_no; ?></td>
																<td class="text-start"><?php echo $ifs_code; ?></td>
																<td class="text-start"><?php echo $create_date; ?></td>
															</tr>
