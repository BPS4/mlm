...
<?php
    ...
    if (isset($_POST['status'])) {
        ...
        mysqli_query($conn, "UPDATE `configuration` SET `is_active`='$status', `update_date`='$current_date' WHERE `id`='$id'");
    ...
    if (isset($_POST['submit_update'])) {
        ...
        $res = mysqli_query($conn, "UPDATE `configuration` SET `remarks`='$message', `update_date`='$current_date' WHERE `id`='$id'");
        if (mysqli_affected_rows($conn)>0) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `configuration` WHERE `id` IN (3,4,5) AND `delete_date` IS NULL ORDER BY `create_date` DESC");
...
<head>
    ...
    <title>Website Maintenance - Admin Dashboard - <?php echo "$name"; ?></title>

    <link rel="canonical" href="maintenance.php">
</head>
                                        ...
                                        <tbody id="tableBody">
                                            ...
                                            foreach ($records as $image) {
                                                ...
                                                if ($key_field == "login_maintain") {
                                                    $key_field = "<span class='badge bg-danger'>MAINTAIN LOGIN</span>";
                                                }
                                        
                                                if ($key_field == "signup_maintain") {
                                                    $key_field = "<span class='badge bg-dark'>MAINTAIN SIGNUP</span>";
                                                }

                                                if ($key_field == "web_maintain") {
                                                    $key_field = "<span class='badge bg-info'>MAINTAIN WEBSITE</span>";
                                                }
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-start"><?php echo $key_field; ?></td>
                                                    <td class="text-start"><?php echo $remarks; ?></td>
                                                    <td>
                                                    ...
                                                    <td class="text-center"><?php echo $update_date; ?></td>
                                                    <td class="text-center">
                                                        <a class="btn btn-warning" href="#" data-bs-toggle="modal" data-bs-target="#ModalEdit" data-id="<?php echo $id; ?>" data-remarks="<?php echo $remarks; ?>" style="margin: 5px;">Edit</a>
                                                        <!-- <form action="" method="POST">
                                                            <input type="hidden" name="id" value="<?php echo $id; ?>">
        ...
        // PASS DATA TO MODAL POPUP
            $(function () {
                $('#ModalEdit').on('show.bs.modal', function (event) {
					...
					document.getElementById("message_edit").value = remarks;
                });
            });
        // PASS DATA TO MODAL POPUP
    </script>
