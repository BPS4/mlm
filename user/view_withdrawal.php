...
<?php
    ...
    // GET SELF INVESTMENT
        ...
        if (mysqli_num_rows($queryselfinvestment) > 0) {
            extract($resselfinvestment); 
        }
    // GET SELF INVESTMENT

    // GET LEVEL 1 INVESTMENT
        $query_level1investment = "SELECT 
        SUM(transaction_amount) as 'level1_investment' 
    FROM
        fund_transaction
    WHERE
        `fund_transaction`.`create_date` >= '$firstdate'
            AND `fund_transaction`.`create_date` <= '$lastdate'
            AND `fund_transaction`.`transaction_type` IN ('fresh' , 'admin_credit')
            AND user_id IN (SELECT 
                user_id
            FROM
                levels
            WHERE
                user_id_up = $user_id AND level = 1)";
        ...
        if (mysqli_num_rows($querylevel1investment) > 0) {
    ...
    // INDIVIDUAL KYC STATUS
        ...
        if ($pan_approved > 0 && $aadhaar_approved > 0 && $photo_approved > 0 && ($cheque_approved > 0 || $passbook_approved > 0)) {
            $kyc_done = 1;
                                                        ...
                                                        <?php
                                                            ...
                                                            foreach ($transactions as $item) {
                                                                ...
                                                                ?>
                                                                    <tr class="no-wrap">
                                                                        <td><?php echo $i; ?></td>
                                                                        <td class="no-wrap"><?php echo $item['from_wallet']; ?></td>
                                                                        <td class="no-wrap">₹ <?php echo number_format($withdrawal_amount,2); ?></td>
                                                                        <td class="no-wrap">₹ <?php echo number_format($admin_charges + $tds,2) ; ?></td>
            ...
            $.ajax({
				...
				success: function(response) {
                    ...
                    switch (response) {
                        ...
                        case 'fund_limit_exceed':
                            showNotif("As per your Limit you can max withdraw only <i>Rs."+allowedwithdrawalamount+"(You can withdraw only 20% of your previous month's investment amount or your first level business.)",'error');
                            break;
